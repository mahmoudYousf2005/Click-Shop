           ...
           {filterCategory.map((item,index)=>(
                <div key={item.id} className="border border-gray-200 rounded-xl p-8 shadow-lg hover:scale-105 transition-all">
                    <div className="flex justify-center  cursor-pointer">
                        <Image className= {`${colors[index % colors.length].color} w-40 mb-4 rounded-full p-1 
                        hover:scale-125 transition-all`}  src={item?.thumbnail} width={0} height={0} unoptimized alt="category img"/>
                    </div>
